...
import { DMRequest, DMResponse, GameState, NPC, Enemy } from '../types';
...
Always respond in valid JSON format.`;
  ...
  async processRequest(request: DMRequest): Promise<DMResponse> {
    try {
      ...
      const response: DMResponse = JSON.parse(responseContent);
      ...
      logger.error('Error processing DM request', { error, request });
  ...
  private buildContextPrompt(request: DMRequest): string {
    ...
    if (game_state.npcs.length > 0) {
      prompt += `\nNPCs:\n${game_state.npcs.map(npc => 
        `- ${npc.name}: ${npc.description} (${npc.attitude})`
      ).join('\n')}\n`;
    }

    if (game_state.enemies.length > 0) {
      prompt += `\nEnemies:\n${game_state.enemies.map(enemy => 
        `- ${enemy.name}: HP ${enemy.health}/${enemy.max_health}, AC ${enemy.armor_class}`
      ).join('\n')}\n`;
    }
  ...
  async generateCharacterBackstory(characterClass: string, characterName: string): Promise<string> {
    try {
      const completion = await this.openai.chat.completions.create({
        model: 'gpt-3.5-turbo',
        messages: [
      ...
      logger.error('Error generating character backstory', { error, characterClass, characterName });
      return 'A mysterious adventurer with an unknown past.';
    }
  }

  async generateAdventureHook(playerLevel: number, playerClasses: string[]): Promise<string> {
    try {
      const completion = await this.openai.chat.completions.create({
        model: 'gpt-3.5-turbo',
        messages: [
      ...
      logger.error('Error generating adventure hook', { error, playerLevel, playerClasses });
